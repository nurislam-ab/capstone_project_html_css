var slider = function (sliderElement) {
    ...
    var init = function () {  
      ...
      [].forEach.call(document.querySelectorAll(sliderElement + ' > *'), function (section) {        
        ...
        var currentSection = document.createElement('span');
        currentSection.classList.add('current-section')
        currentSection.innerText = "0" + index;
        indicator.appendChild(currentSection)

        var sectionCounter = document.createElement('span');
        sectionCounter.classList.add('counter-section')
        sectionCounter.innerText = "/ 05";
        indicator.appendChild(sectionCounter)

        var sectionCounterIcon = document.createElement('span');
        sectionCounterIcon.classList.add('counter-icon')
        indicator.appendChild(sectionCounterIcon)
    ...
    var changeSlide = function (direction) {
      ...
      document.querySelector('a[data-slider-target-index="' + currentSlide +'"]').classList.add('slider__indicator--active');
      // document.querySelector('a.slider__indicator--active').innerText = currentSlide.toString();
    };
    ...
    var elems1 = document.querySelectorAll(".slider__body");
    var elems2 = document.querySelectorAll(".slides");
    var elems3 = document.querySelectorAll(".slider__container");
    var elems4 = document.querySelectorAll(".slider__page");

    if(window.matchMedia('(max-width: 1023px)').matches){
      [].forEach.call(elems1, function(el) {
          el.classList.remove("slider__body");
      });
    }
    ...
    if(window.matchMedia('(max-width: 1023px)').matches){
      [].forEach.call(elems3, function(el) {
          el.classList.remove("slider__container");
      });
    }
